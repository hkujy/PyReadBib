...
def TransitAssignment():
	"""
		process the literature for transit assignment
	"""
	df = pd.read_excel("../TransitAssignmnent/TranstiAssignment.xls")
	with open("PrintAbstracts.md", "w+") as f:
		for i in range(0, len(df["Abstract"])):
			...
			print("2. "+"{0}".format(df['Source Title'][i]), file=f)
			print("> "+"{0}".format(df['Abstract'][i]), file=f)


def test_code():
	"""
		this is for testing and demonstrating the usage
	"""
